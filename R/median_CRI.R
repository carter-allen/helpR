#' A helpR function
#'
#' This function allows you to easily paste a median and credible interval into a 
...
median_CRI <- function(x,na.rm = FALSE,digits = 2,cred = 0.95)
{
    if(!is.numeric(x))
    {
    ...
    else
    {
        ...
        return(paste(round(median(x,na.rm = na.rm),digits = digits),paste("(",int,")",sep = "")))
    }
}